...
class _TestLock(BaseTestCase):
    ...
    def test_lock(self):
        ...
        if sys.hexversion > 0x30e00a6: self.assertTrue(lock.locked())
...
class _TestSharedMemory(BaseTestCase):
    ...
    @unittest.skipIf(sys.hexversion <= 0x30d00a4, "added in 3.13.0a5")
...
class TestStartMethod(unittest.TestCase):
    ...
    def test_get_all_start_methods(self): #FIXME: adjusted for fork preference
        methods = multiprocessing.get_all_start_methods()
        self.assertIn('spawn', methods)
        if sys.platform == 'win32':
            self.assertEqual(methods, ['spawn'])
        elif sys.platform == 'darwin':
            self.assertEqual(methods[0], 'fork') # The default is first. #FIXME
            # Whether these work or not, they remain available on macOS.
            self.assertIn('forkserver', methods)
            self.assertIn('spawn', methods) #FIXME
        else:
            # POSIX
            ...
            self.assertNotIn(methods[0], {'forkserver', 'spawn'}) #FIXME
            #                msg='3.14+ default must not be fork')
            if methods[0] == 'spawn': #FIXME: is never True
...
class TestResourceTracker(unittest.TestCase):
    ...
    @unittest.skipIf(sys.hexversion <= 0x30e04f0, "added in 3.14.5")
    ...
    def test_resource_tracker_mp_fork_reuse_and_prompt_reap(self):
        # gh-146313 / gh-80849: A child started via multiprocess.Process
...
class _TestSpawnedSysPath(BaseTestCase):
    ...
    @unittest.skipIf(sys.hexversion <= 0x30e00f0, "added in 3.14.1")
...
class MiscTestCase(unittest.TestCase):
    ...
    def test_large_pool(self):
        ...
        with open(testfn, 'w', encoding='utf-8') as f:
            f.write(textwrap.dedent('''\
                ...
                def f(x): return x*x
                if __name__ == '__main__':
                    with multiprocess.Pool(200) as p:
    ...
    @unittest.skipIf(sys.hexversion <= 0x30e02f0, "added in 3.14.3")
    ...
    @unittest.skipIf(sys.hexversion <= 0x30e04f0, "added in 3.14.4")
